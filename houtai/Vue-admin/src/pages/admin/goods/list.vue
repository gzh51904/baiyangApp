<template>
  <div style="over-flow:auto">
    ...
    <div class="modifyBox" v-if="MSGbox">
      ...
      <div>
        <span>商品类型</span>
        <input type="text" v-model="goods.type">
        <br>
      </div>
      <div>
        <span>价格</span>
        <input type="text" v-model="goods.cur_price">
        <br>
      </div>
      <div>
        <span>数量</span>
      ...
      <div>
        <span>des</span>
        <input type="text" v-model="goods.des">
      </div>
      <div>
        <span>图片</span>
        <input type="text" v-model="goods.img_url">
      </div>
...
export default {
  ...
  methods: {
    ...
    confirm() {
      ...
      this.goodsList[this.currentIdx].cur_price = this.goods.cur_price
        ? this.goods.cur_price
        : this.goodsList[this.currentIdx].cur_price;
      this.goodsList[this.currentIdx].num = this.goods.num
        ? this.goods.num
        : this.goodsList[this.currentIdx].num;
      this.goodsList[this.currentIdx].type = this.goods.type
        ? this.goods.type
        : this.goodsList[this.currentIdx].type;
      ...
      // id 要转成数字 post请求，goods/?id=1, 1是数字
      id = Number(id);
      let name = this.goodsList[this.currentIdx].name;
      let cur_price = this.goodsList[this.currentIdx].cur_price;
      let num = this.goodsList[this.currentIdx].num;
      let type = this.goodsList[this.currentIdx].type;
      let des = this.goodsList[this.currentIdx].des;
